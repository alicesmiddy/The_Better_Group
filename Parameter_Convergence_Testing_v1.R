...
lambda.hat.2<-mean(th4.cor.ind)
lambda.hat.3<-3
sigmasq.hat<-mean(th5.cor.ind)
beta.hat<-mean(th6.cor.ind)

#Predicted response using the MEDIAN of lambda
r.hat<-E0.hat+((trial$d^(lambda.hat))*Emax.hat)/((trial$d^(lambda.hat))+(ED50.hat+beta.hat*trial$bio1)^(lambda.hat))
#Predicted Response with lambda=median, separating the population with/without biomarker
...
#Predicted response with lambda=mean
r.bio.hat.2<-E0.hat+((trial.biomarker$d^(lambda.hat.2))*Emax.hat)/((trial.biomarker$d^(lambda.hat.2))+(ED50.hat+beta.hat*trial.biomarker$bio1)^(lambda.hat.2))
r.no.bio.hat.2<-E0.hat+((trial.no.biomarker$d^(lambda.hat.2))*Emax.hat)/((trial.no.biomarker$d^(lambda.hat.2))+(ED50.hat+beta.hat*trial.no.biomarker$bio1)^(lambda.hat.2))

#Predicted response with lambda=3
r.bio.hat.3<-E0.hat+((trial.biomarker$d^(lambda.hat.3))*Emax.hat)/((trial.biomarker$d^(lambda.hat.3))+(ED50.hat+beta.hat*trial.biomarker$bio1)^(lambda.hat.3))
r.no.bio.hat.3<-E0.hat+((trial.no.biomarker$d^(lambda.hat.3))*Emax.hat)/((trial.no.biomarker$d^(lambda.hat.3))+(ED50.hat+beta.hat*trial.no.biomarker$bio1)^(lambda.hat.3))

#Comparing predictions with different estimates of lambda
par(mfrow=c(3,1))
plot(trial.biomarker$d,r.bio.hat,col="red",type="l",main="Predicted dose response with"~ lambda~"=median",xlab="Dose amount",ylab="Predicted response")
lines(trial.no.biomarker$d,r.no.bio.hat,col="blue")
legend("topleft",c("Biomarker","No Biomarker"),col=c("red",blue"),lty=c(1,1))

plot(trial.biomarker$d,r.bio.hat.2,col="red",type="l",main="Predicted dose response with"~ lambda~"=mean",xlab="Dose amount",ylab="Predicted response")
lines(trial.no.biomarker$d,r.no.bio.hat.2,col="blue")
legend("topleft",c("Biomarker","No Biomarker"),col=c("red",blue"),lty=c(1,1))

plot(trial.biomarker$d,r.bio.hat.3,col="red",type="l",main="Predicted dose response with"~ lambda~"=3",xlab="Dose amount",ylab="Predicted response")
lines(trial.no.biomarker$d,r.no.bio.hat.3,col="blue")
legend("topleft",c("Biomarker","No Biomarker"),col=c("red",blue"),lty=c(1,1))

#Comparing observed vs Predicted with lambda as the MEDIAN
